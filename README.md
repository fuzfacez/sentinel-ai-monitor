...
Для VPS без домена подготовлен безопасный запуск через SSH-туннель: [SERVER_DEPLOY.md](SERVER_DEPLOY.md). Панель привязывается к `127.0.0.1`, а Telegram-уведомления работают круглосуточно без открытого веб-порта.

Используйте длинный случайный `ADMIN_TOKEN`, не коммитьте `.env`, настройте backup volume `postgres_data`. Запускайте только один экземпляр `app`: APScheduler встроен в процесс. Для горизонтального масштабирования планировщик следует вынести в отдельный worker.
